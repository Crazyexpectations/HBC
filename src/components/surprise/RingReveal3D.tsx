    ...
    <group>
      <Float speed={1.2} rotationIntensity={0.15} floatIntensity={revealed ? 0.3 : 0.7}>
        ...
        >
          ...
          <mesh position={[0, 0.2, 0]} castShadow receiveShadow>
            ...
            <meshStandardMaterial color="#b82d52" roughness={0.35} metalness={0.2} />
          </mesh>
          <mesh position={[0, 0.2, 0]}>
            <boxGeometry args={[0.2, 0.87, 1.32]} />
            <meshStandardMaterial color="#f0b854" roughness={0.25} metalness={0.35} />
          </mesh>
          <mesh position={[0, 0.2, 0]}>
          ...
          <group ref={lidRef} position={[0, 0.62, 0]}>
            <mesh castShadow>
              ...
              <meshStandardMaterial color="#f0b854" roughness={0.3} metalness={0.3} />
            </mesh>
          </group>
        </group>
      </Float>
      ...
      <group ref={ringGroupRef} position={[0, 0.1, 0]} scale={0.001}>
        <mesh rotation={[Math.PI / 2, 0, 0]}>
          ...
          <meshStandardMaterial color="#f0b854" roughness={0.15} metalness={0.9} />
        </mesh>
        {/* the gem is a proper cherry-red stone, on theme for her name */}
        <mesh ref={gemRef} position={[0, 0.26, 0]}>
          <octahedronGeometry args={[0.11, 0]} />
          <meshStandardMaterial color="#e8496b" roughness={0.05} metalness={0.1} emissive="#e0355c" emissiveIntensity={0.5} />
        </mesh>
      </group>

      {revealed && <Sparkles count={30} scale={2.4} size={3} speed={0.5} color="#ffdb94" position={[0, 0.9, 0]} />}
    </group>
  );
}
